...
- **`public/decks/_lite/`** — site-owned, shared by every edition, not an edition itself. `deck-lite.js` + `deck-lite.css` are the deck's mobile lite mode; `publish-deck.ps1` injects a `<link>` and a `<script>` for them into each copied deck's `<head>` (alongside the `noindex` meta), so the edition folders stay verbatim. Read `deck-lite.js`'s header before touching either file — in particular, its list of `window.__cf*` fx guards has to track whatever is in a new edition's `fx/`.
...
- **Phones and tablets never get the embed.** `main.js` gates it on `matchMedia("(max-width: 1024px), (pointer: coarse)")` and shows `.deck-stage__poster` instead, linking to the deck full-screen — the iframe renders a full 1600×900 document (`--deck-scale` is only a paint-time transform), which mobile Safari kills the tab over. `deck-lite.js` uses the same query, so the deck the poster links to loads in lite mode. Test lite from a desktop with `?lite=1` on the deck URL (`?lite=0` forces it off).
