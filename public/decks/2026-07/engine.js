...
(function () {
  ...
  function buildCover() {
    // The cover shows THIS session's date. `C.next` is the FOLLOW-UP edition and
    // belongs on the closing slide only. (Older editions had no `session` block —
    // fall back to `next` so archived decks keep rendering.)
    const now = C.session || C.next;
    // Tonight's themes as emoji chips (meta.topics). Omit the array to hide the row.
    const topics = (C.meta.topics || []).map(t =>
      `<span class="chip chip--topic">${esc(t)}</span>`).join("");
    return slide("cover", "Cover", `
      <div class="cover">
        <div class="cover__main">
          ...
          <div class="row mt2">
            <span class="chip">📅 ${esc(now.dateLabel)} · ${esc(now.time)}</span>
            <span class="chip chip--accent">🎙️ Live & community-driven</span>
          </div>
          ${topics ? `<div class="row cover__topics">${topics}</div>` : ""}
  ...
  /* ---- Tool of the Month: DEMO PREVIEW (the hook, not the deep dive) ------
     Used when `tool.repo` is set. Four things and nothing else: the claim, the
     repo card + QR, two stat tiles, and one animated strip of the agent's
     defenses. Everything explanatory lives in the beats' hover notes, so the
     host can point at a beat instead of the room reading a wall of text.
     Wired up in initToolDemo(). */
  function buildToolPreview(t) {
    const r = t.repo, g = t.beats || {}, nodes = g.nodes || [];

    const tiles = (t.stats || []).map((s, i) => {
      // Only tick up numbers worth watching climb — "1st" / "5" just pop in.
      const m = String(s.big).match(/^(\D*?)(\d{2,})(\D*)$/);
      const big = m ? `<span data-countup="${m[2]}">${esc(m[1])}<i>${esc(m[2])}</i>${esc(m[3])}</span>` : esc(s.big);
      return `
        <article class="dmo-stat" style="--i:${i}">
          <b class="dmo-stat__big">${big}</b>
          <span class="dmo-stat__lbl">${esc(s.label)}</span>
          ${s.sub ? `<span class="dmo-stat__sub">${esc(s.sub)}</span>` : ""}
        </article>`;
    }).join("");

    const gates = nodes.map((n, i) => `
      <button class="dmo-gate${n.stop ? " is-stop" : ""}" style="--i:${i}" type="button">
        <span class="dmo-gate__ic">${esc(n.icon || "🔒")}</span>
        <span class="dmo-gate__name">${esc(n.name)}</span>
        ${n.note ? `<span class="dmo-gate__note">${esc(n.note)}</span>` : ""}
      </button>`).join("");

    // Beat ignition marches in step with the task crossing the lane.
    const dur = 3.6, step = nodes.length > 1 ? dur / (nodes.length - 1) : dur;

    const qr = t.url ? `
      <aside class="tof-reg panel dmo-qr">
        <div class="dmo-qr__lbl">Read the code</div>
        ${qrCard(t.url, 132, 5)}
        ${linkmono(t.url)}
      </aside>` : "";
    ...
    return slide("tool", "Tool of the Month", `
      ...
      ${t.blurb ? `<p class="lead mt dmo-lead">${esc(t.blurb)}</p>` : ""}
      <div class="dmo-grid mt">
        <div class="dmo-col">
          <article class="panel dmo-repo">
            <div class="dmo-repo__bar">
              <span class="dmo-repo__gh">⎇</span>
              <span class="dmo-repo__path">${esc(r.owner)}<b>/${esc(r.name)}</b></span>
              <span class="dmo-repo__badge">public</span>
              ${t.who ? `<span class="dmo-repo__who"><span class="tof-feat__dot"></span>${esc(t.who)}</span>` : ""}
            </div>
            ${r.desc ? `<p class="dmo-repo__desc">${esc(r.desc)}</p>` : ""}
            ${(r.stack || []).length ? `<div class="dmo-stack">${r.stack.map(s => `<span class="chip">${esc(s)}</span>`).join("")}</div>` : ""}
          </article>
          <div class="dmo-stats">${tiles}</div>
        </div>
        ${qr}
      </div>
      ${nodes.length ? `
      <section class="panel dmo-run mt" data-run style="--dur:${dur}s;--step:${step.toFixed(2)}s">
        <div class="dmo-run__head">
          <span class="dmo-run__lbl">▶ ${esc(g.title || "One task, end to end")}</span>
          ${g.hint ? `<span class="dmo-run__hint muted">${esc(g.hint)}</span>` : ""}
          <button class="dmo-replay" data-run-replay type="button">↻ replay</button>
        </div>
        <div class="dmo-lane"><span class="dmo-rail"></span><span class="dmo-beam"></span><span class="dmo-pkt">📄</span></div>
        <div class="dmo-track">${gates}</div>
      </section>` : ""}`);
  }

  /* ---- Demo preview: autoplay the run on entry + count up the stat tiles -
     Same lifecycle trick as initAgenda — no per-slide hooks exist, so watch
     the slide's .active class. Animations are CSS, gated on .dmo-run.go, so
     replaying is just: drop the class, force a reflow, put it back. */
  function initToolDemo() {
    const slideEl = $('.slide[data-id="tool"]'); if (!slideEl) return;
    const run = $("[data-run]", slideEl); if (!run) return;
    const nums = [...slideEl.querySelectorAll("[data-countup] i")];

    function countUp() {
      nums.forEach(el => {
        const to = +el.parentNode.dataset.countup, t0 = performance.now(), d = 1100;
    ...
    function play() {
      run.classList.remove("go"); void run.offsetWidth; run.classList.add("go");
      countUp();
    }
    const replay = $("[data-run-replay]", run);
    if (replay) replay.addEventListener("click", play);
    new MutationObserver(() => { if (slideEl.classList.contains("active")) play(); })
      .observe(slideEl, { attributes: true, attributeFilter: ["class"] });
    if (slideEl.classList.contains("active")) play();
  }

  function buildTool() {
    const t = C.tool;
    // Demo preview (a public repo) > rich "triple feature" line-up > the simple
    // single-tool card (name + blurb + QR).
    if (t.repo) return buildToolPreview(t);
  ...
  /* ---- A hand-built Slack thread (used as the ambient-AI hero) ----------
     Beats a screenshot: crisp at projector size, no cropping, and we control
     how much text is on screen. Shape comes from spotlight.emmi.slack. */
  function slackMock(s) {
    const at = t => esc(t).replace(/@Claude/g, '<span class="sk-at">@Claude</span>');
    const msgs = s.messages.map(m => `
      <div class="sk-msg${m.push ? " sk-msg--push" : ""}">
        <span class="sk-av${m.bot ? " sk-av--bot" : ""}"${m.bot ? "" : ` style="--av:${esc(m.color || "#4A6FA5")}"`}
              aria-hidden="true">${m.bot ? "✳" : esc(m.initial || m.who[0])}</span>
        <div class="sk-body">
          <div class="sk-meta"><b>${esc(m.who)}</b>${m.bot ? '<span class="sk-app">APP</span>' : ""}
            <span class="sk-time">${esc(m.time)}</span></div>
          ${m.text ? `<p class="sk-txt">${at(m.text)}</p>` : ""}
          ${m.bullets ? `<ul class="sk-list">${m.bullets
            .map(b => `<li><b>${esc(b.k)}</b> — ${esc(b.v)}</li>`).join("")}</ul>` : ""}
          ${m.link ? `<span class="sk-link">${esc(m.link)}</span>` : ""}
        </div>
        ${m.note ? `<span class="sk-note">⚡ ${esc(m.note)}</span>` : ""}
      </div>`).join("");
    return `<div class="sk" role="img" aria-label="${esc(s.alt || "A Slack thread where Claude answers and later speaks up unprompted")}">
      <div class="sk__bar"><span class="sk__ch"># ${esc(s.channel)}</span>
        <span class="sk__sub">${esc(s.thread || "Thread")}</span></div>
      <div class="sk__body">${msgs}</div>
    </div>`;
  }
  ...
  function buildEmmi() {
    ...
    const details = e.timeline.map((t, i) => `
      <p class="tl-detail${i === 0 ? " on" : ""}" data-tld="${i}">${esc(t.body)}${
        t.adopt ? ` <span class="tl-adopt">📈 ${esc(t.adopt)}</span>` : ""}</p>`).join("");
    const uses = e.uses.map(u => `<span class="chip">${esc(u)}</span>`).join("");
    return slide("emmi", e.navTitle || "Story of the month", `
      <span class="kicker">${esc(e.kicker)}</span>
      <div class="dd-head">
        <h2>${esc(e.title)} <span class="grad">${esc(e.emoji || "🤝")}</span></h2>
        <span class="chip chip--accent dd-tag">${esc(e.tag)}</span>
      </div>
      <div class="emmi-grid mt">
        <figure class="emmi-hero${e.slack ? " emmi-hero--sk" : ""}">
          ${e.slack ? slackMock(e.slack)
                    : `<img src="${esc(e.hero)}" alt="${esc(e.heroAlt || e.title)}">`}
          <figcaption>${esc(e.deal)}</figcaption>
        </figure>
        <div class="panel emmi-what" data-seg-group>
          <div class="seg">
            <button class="seg-btn on" data-seg="0" type="button">${esc(e.what.before.tab || "🐌 Before")}</button>
            <button class="seg-btn" data-seg="1" type="button">${esc(e.what.after.tab || "⚡ After")}</button>
          </div>
          <div class="seg-body">
      ...
      ${(e.note || e.link) ? `<div class="emmi-foot">
        ${e.note ? `<p class="emmi-note warn">⚠ ${esc(e.note)}</p>` : ""}
        ${e.link ? `<div class="nc-qr emmi-link">${qrCard(e.link, 68, 3)}${linkmono(e.link)}</div>` : ""}
      </div>` : ""}`);
  }
  ...
  /* ---- Deep dive 4: Model face-off (cards + benchmark bar charts) -------
     One card per model (the colour legend) and one small bar chart per
     benchmark, so the gaps are seen rather than read. Click a card to trace
     that model through every chart. Numbers live in spotlight.faceoff. */
  function buildFaceoff() {
    const f = C.spotlight && C.spotlight.faceoff;
    if (!f || !(f.models && f.models.length)) return "";
    const by = Object.fromEntries(f.models.map(m => [m.key, m]));

    const cols = f.models.map(m => `
      <button class="fo-col panel" data-fo-key="${esc(m.key)}" type="button" style="--c:${esc(m.color)}">
        <div class="fo-col__top">
          <span class="fo-flag">${esc(m.flag || "🤖")}</span>
          <div class="fo-id"><b>${esc(m.name)}</b><span>${esc(m.vendor)}</span></div>
          ${m.hot ? '<span class="fo-new">NEW</span>' : ""}
        </div>
        <dl class="fo-specs">
          <div><dt>Context</dt><dd>${esc(m.ctx)}</dd></div>
          <div><dt>Weights</dt><dd>${esc(m.weights || (m.open ? "Open" : "Closed"))}</dd></div>
          <div><dt>$/1M in</dt><dd>$${esc(m.pin)}</dd></div>
          <div><dt>$/1M out</dt><dd>$${esc(m.pout)}</dd></div>
          <div class="fo-specs__wide"><dt>Released</dt><dd>${esc(m.released)}</dd></div>
        </dl>
        <span class="fo-win">🏆 ${esc(m.win)}</span>
      </button>`).join("");

    const charts = (f.charts || []).map(c => {
      const lo = c.lo ?? 0, hi = c.hi ?? 100;
      const rows = c.bars.map((b, i) => {
        const m = by[b.m] || {}, has = b.v != null;
        const pct = has ? Math.max(3, Math.min(100, ((b.v - lo) / (hi - lo)) * 100)) : 0;
        const val = has ? `${c.prefix || ""}${b.v}${c.suffix || ""}` : (b.na || "n/a");
        return `<div class="fo-row${i === 0 && has ? " win" : ""}${has ? "" : " na"}"
                     data-m="${esc(b.m)}" style="--c:${esc(m.color)};--w:${pct.toFixed(1)}%;--i:${i}">
          <span class="fo-row__n">${esc(m.short || b.m)}</span>
          <div class="fo-track"><span class="fo-fill"></span></div>
          <b class="fo-row__v">${esc(val)}</b>
        </div>`;
      }).join("");
      return `<div class="fo-chart panel">
        <div class="fo-chart__hd"><b>${esc(c.name)}</b><span class="fo-chart__hint">${esc(c.hint)}</span></div>
        <span class="fo-chart__sub">${esc(c.sub)}</span>
        <div class="fo-rows">${rows}</div>
      </div>`;
    }).join("");

    return slide("faceoff", f.navTitle || "Model face-off", `
      <span class="kicker">${esc(f.kicker)}</span>
      <div class="dd-head">
        <h2>${esc(f.title)} <span class="grad">${esc(f.emoji || "⚔️")}</span></h2>
        <span class="chip chip--accent dd-tag">${esc(f.tag)}</span>
      </div>
      <p class="lead mt fo-intro">${hot(f.intro)}</p>
      <div class="fo-wrap">
        <div class="fo-grid">${cols}</div>
        <div class="fo-charts">${charts}</div>
      </div>
      <p class="dd-foot muted">⚠ ${esc(f.foot)} <span class="fo-src">${esc(f.source)}</span></p>`);
  }

  /* ---- Deep dive 5: the open-weights letter (signatory wall) ------------
     The whole slide is built around one image: fifty logos on one page. Tiles
     carry the mark on a white plate (Simple Icons are single-path black, so a
     plate is the only way they stay legible across all six deck themes) and the
     brand colour rides on a 3px accent bar. Signatories with no recognisable
     mark get a typographic plate instead — same tile, no fake logo.
     Clicking a filter dims the wall down to the day-two joiners or lights up
     the two empty chairs; wired in initSpotlight(). */
  function buildOpenWeights() {
    const o = C.spotlight && C.spotlight.openweights;
    if (!o || !(o.wall && o.wall.length)) return "";

    // One tile. `ghost` renders the "didn't sign" variant (dashed, desaturated).
    const tile = (s, i, ghost) => {
      const cls = ["ow-tile"];
      if (s.logo) cls.push("ow-tile--logo"); else cls.push("ow-tile--word");
      if (s.lead) cls.push("ow-tile--lead");
      if (ghost) cls.push("ow-tile--out");
      const mark = s.logo
        ? `<span class="ow-plate"><img src="assets/news/openweights/${esc(s.logo)}.svg"
             alt="" aria-hidden="true" loading="lazy"></span>`
        : `<span class="ow-plate ow-plate--word"><b>${esc(s.n)}</b></span>`;
      return `<li class="${cls.join(" ")}" data-late="${s.late ? "1" : "0"}"
                  style="--c:${esc(s.c || "var(--muted)")};--i:${i}"
                  title="${esc(s.why || s.n)}">
        ${mark}
        <span class="ow-name">${esc(s.n)}</span>
        ${s.late ? '<span class="ow-badge">+24h</span>' : ""}
        ${s.lead ? '<span class="ow-badge ow-badge--lead">wrote it</span>' : ""}
        ${ghost ? '<span class="ow-badge ow-badge--out">✕</span>' : ""}
      </li>`;
    };

    const wall = o.wall.map((s, i) => tile(s, i, false)).join("");
    const out = (o.absent || []).map((s, i) => tile(s, i, true)).join("");
    const filters = (o.filters || []).map((f, i) =>
      `<button class="seg-btn ow-filter${i === 0 ? " on" : ""}" data-ow-f="${esc(f.k)}"
               type="button">${esc(f.label)}</button>`).join("");

    // Signature meter: solid block = day one, striped = the overnight half.
    const m = o.meter || {};
    const d1 = ((m.day1 || 0) / (m.total || 1)) * 100;
    const meter = `
      <div class="ow-meter panel">
        <div class="ow-meter__hd"><b>${esc(m.head || "Signatures")}</b>
          <span class="ow-meter__big" data-ow-count="${esc(m.total || 0)}">0</span></div>
        <div class="ow-bar" style="--d1:${d1.toFixed(1)}%">
          <span class="ow-bar__d1"></span><span class="ow-bar__d2"></span>
        </div>
        <div class="ow-meter__legend">
          <span><i class="ow-key ow-key--d1"></i>${esc(m.d1Label || "")}</span>
          <span><i class="ow-key ow-key--d2"></i>${esc(m.d2Label || "")}</span>
        </div>
        ${m.note ? `<p class="ow-meter__note">${hot(m.note)}</p>` : ""}
      </div>`;

    const rhyme = o.rhyme ? `
      <div class="ow-rhyme panel">
        <b class="ow-rhyme__hd">${esc(o.rhyme.head)}</b>
        ${o.rhyme.rows.map(r => `
          <div class="ow-era${r.now ? " ow-era--now" : ""}">
            <span class="ow-era__tag">${esc(r.icon)} ${esc(r.era)}</span>
            <p class="ow-era__claim">${esc(r.claim)}</p>
            <div class="ow-era__beats">${r.beats.map(b =>
              `<span class="ow-beat">${esc(b)}</span>`).join('<i class="ow-beat__link"></i>')}</div>
            <p class="ow-era__end">${esc(r.end)}</p>
          </div>`).join("")}
      </div>` : "";

    const asks = (o.asks || []).length ? `
      <div class="ow-asks">
        <b class="ow-asks__hd">${esc(o.asksHead || "The ask")}</b>
        <div class="ow-asks__grid">${o.asks.map(a => `
          <div class="ow-ask">
            <span class="ow-ask__i">${esc(a.icon)}</span>
            <b>${esc(a.head)}</b>
            <p>${esc(a.body)}</p>
          </div>`).join("")}</div>
      </div>` : "";

    return slide("openweights", o.navTitle || "Open weights letter", `
      <span class="kicker">${esc(o.kicker)}</span>
      <div class="dd-head">
        <h2>${esc(o.title)} <span class="grad">${esc(o.emoji || "📜")}</span></h2>
        <span class="chip chip--accent dd-tag">${esc(o.tag)}</span>
      </div>
      <p class="lead mt fo-intro">${hot(o.intro)}</p>
      <div class="ow-top">
        <div class="ow-col">
          <article class="tweet ow-tweet">
            <div class="tweet__top">
              <div class="tweet__av">${esc(o.tweet.avatar)}</div>
              <div class="tweet__id"><b>${esc(o.tweet.name)} <span class="vbadge">✔</span></b>
                <span>${esc(o.tweet.handle)} · ${esc(o.tweet.sub)}</span></div>
              <div class="tweet__x">𝕏</div>
            </div>
            <p class="tweet__text">${esc(o.tweet.text)}</p>
            <div class="tweet__meta">${esc(o.tweet.date)} · <b>${esc(o.tweet.views)}</b></div>
            ${o.tweet.foot ? `<p class="ow-tweet__foot">📎 ${esc(o.tweet.foot)}</p>` : ""}
          </article>
          ${meter}
        </div>
        <div class="ow-col">
      ...
      <div class="ow-wallbox" data-ow-wall data-ow-view="all">
        <div class="ow-wall__hd">
          <b>${esc(o.wallHead || "Signatories")}</b>
          <span class="ow-wall__hint">${esc(o.wallHint || "click to filter")}</span>
          <div class="seg ow-seg">${filters}</div>
        </div>
        <ul class="ow-wall">${wall}</ul>
        <div class="ow-foot">
          ${out ? `<div class="ow-outbox">
            <span class="ow-out__lbl">
              <b class="ow-out__hd">${esc(o.absentHead || "Didn't sign")}</b>
              ${o.absentNote ? `<small>${esc(o.absentNote)}</small>` : ""}
            </span>
            <ul class="ow-wall ow-wall--out">${out}</ul>
          </div>` : ""}
          <p class="warn ow-why">⚠ ${esc(o.why)}</p>
          ${o.link ? `<div class="nc-qr ow-qr">${qrCard(o.link, 56, 3)}${linkmono(o.link)}</div>` : ""}
        </div>
      </div>
      <p class="dd-foot muted"><span class="fo-src">${esc(o.source)}</span></p>`);
  }
  ...
  function initSpotlight() {
    ...
    document.addEventListener("click", e => {
      ...
      // 1b) Open-weights wall — filter the signatories (all / day two / absent).
      // MUST be tested before the generic .seg-btn branch below: these buttons
      // borrow the segmented-control look, but they filter a grid instead of
      // swapping panes, so they have no [data-seg-group] ancestor.
      const owF = e.target.closest(".ow-filter");
      if (owF) {
        const box = within(owF, "[data-ow-wall]");
        if (box) {
          box.querySelectorAll(".ow-filter").forEach(b => b.classList.toggle("on", b === owF));
          box.dataset.owView = owF.dataset.owF;
        }
        return;
      }
      ...
      if (segBtn) {
        ...
        if (!grp) return;                       // a seg-styled button from elsewhere
      ...
      // 6b) Model face-off — click a card to trace that model through the charts
      const foCol = e.target.closest(".fo-col");
      if (foCol) {
        const wrap = within(foCol, ".fo-wrap");
        const key = foCol.dataset.foKey, off = foCol.classList.contains("on");
        wrap.querySelectorAll(".fo-col").forEach(c => c.classList.toggle("on", !off && c === foCol));
        wrap.querySelectorAll(".fo-row").forEach(r =>
          r.classList.toggle("dim", !off && r.dataset.m !== key));
        return;
      }
  ...
  function render() {
    const html = [
      ...
      // This edition's Breaking News: ambient AI, the open-weights letter, the
      // model face-off.
      // buildLoop()/buildHarness() are last edition's deep-dive moulds — those
      // topics live only in the retrospective (buildNews) now, so they're not
      // rendered as their own slides this month. Re-add them when a new edition
      // fills spotlight.loop / spotlight.harness.
      buildNews(), buildEmmi(), buildOpenWeights(), buildFaceoff(), buildToolRecap(), buildTool(), buildOutro(),
    ].join("");
    $("#deck").innerHTML = html;
  }
  ...
  /* ---- Numbers that tick up when their slide opens ----------------------
     Any element with data-ow-count="50" counts 0 → 50 the first time its slide
     becomes active (the signature counter on the open-weights slide). */
  function countUp(slide) {
    if (!slide) return;
    slide.querySelectorAll("[data-ow-count]").forEach(el => {
      if (el.dataset.counted) return;
      el.dataset.counted = "1";
      const to = +el.dataset.owCount, t0 = performance.now(), dur = 1200;
      (function frame(now) {
        const k = Math.min(1, (now - t0) / dur);
        el.textContent = Math.round(to * (1 - Math.pow(1 - k, 3)));
        if (k < 1) requestAnimationFrame(frame);
