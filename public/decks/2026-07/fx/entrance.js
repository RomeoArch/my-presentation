/* =============================================================================
   ...
   (B) "The vanish" — finds ONE punchline word on the cover (meta.glitchWord in
       content.js, e.g. "35%"; falls back to June's "fable-less") and, every
       ~6–9s while the cover is active, plays a quick (~700ms) chromatic glitch:
       RGB split (::before/::after via data-text), flicker, fades the word to
       near-zero ("vanishes"), then snaps back ("reforms"). Never alters text.
...
(function () {
  ...
  // Which punchline word vanishes: meta.glitchWord in content.js, else the
  // June-2026 default. Matching is normalized (case/punctuation-insensitive).
  function glitchTarget() {
    var w = "";
    try { w = (window.CODEFORCE && window.CODEFORCE.meta &&
               window.CODEFORCE.meta.glitchWord) || ""; } catch (e) {}
    return normalize(w) || "fable-less";
  }

  function findFableSpan(cover) {
    if (!cover) return null;
    var want = glitchTarget();
    var hots = cover.querySelectorAll(".hot");
    for (var i = 0; i < hots.length; i++) {
      if (normalize(hots[i].textContent) === want) return hots[i];
    }
    return null;
  }
